...
# no caso, esta parte é para o Alterar permissões
		aoo=$( dialog				\
					...
					2 "Ódio" )
				case $aoo in
					1) dialog --backtitle "ROKUKISHI PROJECT"--ok-label Continuar --msgbox "Acesso permitido" 0 0; bash /primeirosemestre/.config/gerenciadordearquivos/aper.sh ;;
					2) dialog --backtitle "ROKUKISHI PROJECT" --ok-label Continuar --title "Acesso Negado" --msgbox "Onde não há amor, não há acesso" 0 0 ; proxima;;
					*) proxima;;
				esac
...
case $opcao in
	...
	4) adir ;;
	5) vera ;;
	6) bash /primeirosemestre/.config/gerenciadordearquivos/list.sh ;;
	7) manual ;;
	8) edir ;;
	9) copa ;;
	10) mova ;;
...
case $opcao in
	...
	7) desc ;;
	8) edit ;;
	9) cd ..; bash /primeirosemestre/.config/menu.sh;;
	10) cd /; bash /primeirosemestre/.config/menu.sh;;
	11) diratual;;
	*) menu;;
esac
...
case $? in
	0) dialog --backtitle "ROKUKISHI PROJECT" --ok-label Continuar --msgbox "O diretório '$arquivo' já existe" 0 0; cd /primeirosemestre; menu;;
...
# Se retornar 0 o diretório existe, voltando para o diretório anterior acessado (Provavelmente /primeirosemestre)
